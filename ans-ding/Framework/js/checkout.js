...
function reservationsF() {
  ...
  let resContent = `<h2>Reservations</h2>${filterBtn}<div id='reservationsDiv'></div>`;
  $("#reservationsNav").addClass("active");
  $("#content").html("<div id='contentReservation'></div>");
  $("#contentReservation").html(resContent);
  ...
  $("#reservationsDiv").html(`<table id='reservations'>${tableHeaderRow}</table>`);
  for (let i = 0; hotelRooms.length > i; i++) {
...
function dateF(date, style) {
  ...
  if (style === "American") {
    return `${d.getFullYear()}-${month_of_the_year(d)}-${day_of_the_month(d)}`;
  }
}
...
function filter() {
  ...
  for (let i = 0; nameArray.length > i; i++) {
